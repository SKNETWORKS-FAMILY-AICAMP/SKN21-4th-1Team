...
또한, 질문에서 **사용자가 직접 언급한** 법령명을 정확히 추출하세요. (추론 금지)

## 판단 기준 (Ambiguity Check)
(category="노동법"인 경우에만 적용)
...
- 단순 법령/제도 정의 질문 (예: "해고예고수당이 뭔가요?", "최저임금 얼마인가요?", "근로기준법 제34조 알려줘")
- 이미 정보가 포함된 질문 (예: "5인 이상 사업장인데 부당해고 당했습니다.")
- 노동법 외 일반/일상 질문 (is_ambiguous: False)
...
- **is_ambiguous**: True (정보 부족) / False (답변 가능)
